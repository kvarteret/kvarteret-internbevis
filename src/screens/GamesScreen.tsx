...
function getClockLabelClass(state: ChessTimerState, player: ChessPlayer): string {
  ...
  return ['mb-1.5 font-inter-semibold text-sm', isHighlighted ? 'text-surface' : 'text-text-secondary'].join(' ');
}
...
export function GamesScreen({ navigation }: NativeStackScreenProps<RootStackParamList, 'Games'>): React.JSX.Element {
  ...
  const winnerLabel = timerState.winner ? t('chessWinner', { winner: t(getWinnerLabelKey(timerState.winner)) }) : null;
      ...
      <ScrollView className="flex-1" contentContainerClassName="flex-grow gap-4 p-4">
        <View className="flex-row gap-2.5">
          <Pressable
            className={[
              ...
              mode === 'd6' ? 'border-text-primary bg-text-primary' : 'border-border bg-surface',
            ]
              .filter(Boolean)
              .join(' ')}
            onPress={() => setMode('d6')}
          >
            <Text className={mode === 'd6' ? 'font-inter-semibold text-[15px] text-surface' : 'font-inter-semibold text-[15px] text-text-primary'}>
              {t('gamesDice')}
            </Text>
          </Pressable>
          ...
          <Pressable
            className={[
              'flex-1 items-center justify-center rounded-xl border px-3 py-3',
              mode === 'chess' ? 'border-text-primary bg-text-primary' : 'border-border bg-surface',
            ]
              .filter(Boolean)
              .join(' ')}
            onPress={() => setMode('chess')}
          >
            <Text className={mode === 'chess' ? 'font-inter-semibold text-[15px] text-surface' : 'font-inter-semibold text-[15px] text-text-primary'}>
              {t('gamesChessTimer')}
            </Text>
          </Pressable>
        </View>
        ...
        {mode === 'd6' ? (
          <View className="gap-3 rounded-card border border-border bg-surface p-4">
            <Text className="font-inter-bold text-2xl text-text-primary">{t('gamesDice')}</Text>
            <Text className="text-center font-inter-bold text-7xl text-text-primary">{diceValue}</Text>
            <Text className="text-center font-inter text-sm text-text-secondary">{t('gamesDiceRolls', { count: diceRollCount })}</Text>

            <Pressable className="items-center rounded-xl border border-text-primary bg-text-primary py-3" onPress={handleRollDice}>
              <Text className="font-inter-bold text-base text-surface">{t('gamesRollD6')}</Text>
            </Pressable>
          </View>
        ) : (
          <View className="gap-3 rounded-card border border-border bg-surface p-4">
            <Text className="font-inter-bold text-2xl text-text-primary">{t('gamesChessTimer')}</Text>

            <Pressable className="gap-3" onPress={handleChessBoxTap}>
              <View className="flex-row items-center justify-between">
                <Text className="font-inter-semibold text-base text-text-secondary">{t('chessTimeControl')}</Text>
                <Text className="font-inter-semibold text-base text-text-secondary">{t('chessMoves', { count: timerState.moveCount })}</Text>
              </View>

              <View className={getClockCardClass(timerState, 'white')}>
                <Text className={getClockLabelClass(timerState, 'white')}>{t('chessWhite')}</Text>
                <Text className={getClockValueClass(timerState, 'white')}>{formatClock(timerState.whiteMs)}</Text>
              </View>

              <View className={getClockCardClass(timerState, 'black')}>
                <Text className={getClockLabelClass(timerState, 'black')}>{t('chessBlack')}</Text>
                <Text className={getClockValueClass(timerState, 'black')}>{formatClock(timerState.blackMs)}</Text>
              </View>

              {winnerLabel ? <Text className="font-inter-bold text-base text-[#1B3A0A]">{winnerLabel}</Text> : null}
            </Pressable>

            <View className="mt-1 gap-2.5">
              <Pressable className="items-center rounded-xl border border-text-primary bg-text-primary py-3" onPress={handleToggleTimer}>
                <Text className="font-inter-bold text-base text-surface">{timerState.isRunning ? t('chessPause') : t('chessStart')}</Text>
              </Pressable>

              <Pressable className="items-center rounded-xl border border-border bg-surface py-3" onPress={handleResetTimer}>
                <Text className="font-inter-semibold text-base text-text-primary">{t('chessReset')}</Text>
              </Pressable>
            </View>
          </View>
        )}
