...
export const resetTimer = (initialMs: number, incrementMs = 0): ChessTimerState => {
...
export const tick = (state: ChessTimerState, elapsedMs: number): ChessTimerState => {
...
export const completeMove = (state: ChessTimerState): ChessTimerState => {
    if (!state.isRunning || state.winner) {
        return state
    }
...
export const toggleStartPause = (state: ChessTimerState): ChessTimerState => {
    if (state.winner) {
        return state
    }
...
export const pause = (state: ChessTimerState): ChessTimerState => {
    if (!state.isRunning || state.winner) {
        return state
    }
